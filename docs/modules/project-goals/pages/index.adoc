...
For situations where direct validation of an SBOM may not be so easy, such as optional or alternative data representations in manifests, we'd like to at the very least return feedback on what problems this would cause in what situations, and offer guidance and solutions to address these.
