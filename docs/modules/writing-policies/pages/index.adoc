...
This section we'll dive into the basic concepts of policy evaluation at a high-level.
...
A rules is a self-contained condition that belongs to a policy. Rules are what make up each policy and can be considered the smallest unit of evaluation.
...
An evaluation the output produced by EC/Conforma/OPA with Mequal after running a set of policies against a specific input. It can be in either json or yaml format and contains what violations or warnings have been raised against these policies for the input. This evaluation can be context-specific depending on the policies it runs and the type of input that is passed into the SBOM. (For example different policy checks for different SBOM versions, SBOM types, etc.)

The violations help with our validation of the SBOMs to make sure they comply with the policy and fail the evaluation if there are any policies the SBOM doesn't comply with. These are more related to the basic important rules the SBOM must comply with to be considered valid.

The warnings are more about giving feedback to related to the evaluation against certain policies without necessarily rejecting the SBOM input. They anticipate certain information that each SBOM may not necessarily have to represent, and provide guidance on how to include this information on the SBOM. It also tries to gauge the quality of the SBOM based on what it anticipates and that is actually found within the SBOM in the form of levels or points. 
...
=== Folder structures and corrsponding package hierarchy
...
<7> All *prerequisite conditions* needed for the evaluation to be performed go in this function
...
<12> Before each rule, the *prerequisite conditions* functions should be called to ensure evaluation is only done on inputes that satisfy the prerequisites.
<13> *Helper function* for passing information as output.
<14> String values can be passed into the empty array in the helper function to populate the '%s' variables in the failure_msg metadata
